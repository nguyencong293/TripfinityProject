...
import { getRestaurantsByProvider } from "../../../services/restaurantService";
import ConfirmModal from "../../../components/common/ConfirmModal";

const ListRestaurantBookingPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load restaurants and bookings
        const [restaurantsData, bookingsRes] = await Promise.all([
          getRestaurantsByProvider(provider.providerId),
          api.get<RestaurantBookingDTO[]>(`/restaurant-bookings/provider/${provider.providerId}`)
        ]);
  ...
  const columns: Array<{ key: ColumnKey; label: string }> = useMemo(
    () => [
      ...
      { key: "restaurantName", label: t("restaurant_name") || "Nhà hàng" },
      { key: "userName", label: t("customer_name") || "Tên khách" },
      { key: "userPhone", label: t("phone_number") || "Số điện thoại" },
      { key: "reservationDate", label: t("reservation_date") || "Ngày đặt bàn" },
      { key: "reservationTime", label: t("reservation_time") || "Giờ đặt bàn" },
  ...
  const formatTime = (s?: string) => s ? s.substring(0, 5) : "N/A"; // HH:mm:ss -> HH:mm
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      if (userStr) {
        ...
        const provider = await getProviderByUserId(user.userId);
        if (provider?.providerId) {
          const bookingsRes = await api.get<RestaurantBookingDTO[]>(`/restaurant-bookings/provider/${provider.providerId}`);
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      no_show: dark ? "bg-gray-500/20 text-gray-400 border-gray-500/30" : "bg-gray-100 text-gray-700 border-gray-300",
    };
    return colors[status] || (dark ? "bg-gray-500/20 text-gray-300" : "bg-gray-100 text-gray-700");
  };

  const cell = (b: RestaurantBookingDTO, key: ColumnKey): React.ReactNode => {
    ...
    switch (key) {
      ...
      case "restaurantName":
        return restaurant?.title || `Restaurant ID: ${b.restaurantId}`;
      case "userName":
        return user?.fullName || `User ID: ${b.userId}`;
      case "userPhone":
        return user?.phoneNumber || "-";
      case "reservationDate":
        return formatDate(b.reservationDate);
      case "reservationTime":
        return formatTime(b.reservationTime);
      case "guests":
        return `${b.numAdults} ${t("adults_suffix") || "người"}`;
      case "totalPrice":
        return (
      ...
      case "actions": {
        ...
        return (
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigate(`/supplier/service/restaurant/bookings/${b.bookingId}`)}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className={`text-3xl font-bold ${dark ? "text-white" : "text-gray-900"}`}>
                {t("booking_list_title") || "Danh sách đặt bàn"}
              </h1>
...
export default ListRestaurantBookingPage;
