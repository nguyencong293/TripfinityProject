...
import { getRestaurantReviewById } from "../../../services/restaurantService";
import {
  createReviewReply,
  getReviewReplies,
  updateReviewReply,
...
import type { RestaurantReviewDTO, UserDTO } from "../../../types";

const ReviewDetailPage: React.FC = () => {
  const navigate = useNavigate();
  const { reviewId } = useParams<{ reviewId: string }>();
  const [review, setReview] = useState<RestaurantReviewDTO | null>(null);
  ...
  useEffect(() => {
    const loadReviewData = async () => {
      ...
      try {
        ...
        // Load review
        const reviewData = await getRestaurantReviewById(parseInt(reviewId));
        ...
        // Load replies
        const repliesData = await getReviewReplies("restaurant", parseInt(reviewId), currentUserId);
        setReplies(repliesData);

        // Load review like count and status
        const likeCount = await getLikeCount("restaurant", parseInt(reviewId));
        setReviewLikeCount(likeCount);

        const isLiked = await checkIsLiked(currentUserId, "restaurant", parseInt(reviewId));
