...
import { getHotelsByProvider, getHotelRatingSummaryByHotel } from "../../../services/hotelService";
import type { HotelDTO, HotelRatingSummaryDTO } from "../../../types";

interface HotelWithRating extends HotelDTO {
  ratingSummary?: HotelRatingSummaryDTO;
}
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Fetch rating summary for each hotel
        const hotelsWithRatings = await Promise.all(
          hotelsData.map(async (hotel) => {
            try {
              if (!hotel.hotelId) return null;
              const summary = await getHotelRatingSummaryByHotel(hotel.hotelId);
              return { ...hotel, ratingSummary: summary };
            } catch {
              return { ...hotel, ratingSummary: undefined };
            }
          })
        );

        setHotels(hotelsWithRatings.filter((h): h is HotelWithRating => h !== null));
      } catch (error) {
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredHotels.map((hotel) => {
                const rating = hotel.ratingSummary?.avgRating || 0;
                const totalReviews = hotel.ratingSummary?.totalReviews || 0;
                const statusColor =
                  rating >= 4.5
                      ...
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10">
                          <img
                            ...
                            src={hotel.thumbnailUrl || "/placeholder.jpg"}
                            alt={hotel.title}
                          />
                        </div>
                        <div className="ml-4">
