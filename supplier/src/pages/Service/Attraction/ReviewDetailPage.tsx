...
import { getAttractionReviewById } from "../../../services/attractionService";
import {
  createReviewReply,
  getReviewReplies,
  updateReviewReply,
...
const ReviewDetailPage: React.FC = () => {
  ...
  const [review, setReview] = useState<AttractionReviewDTO | null>(null);
  ...
  useEffect(() => {
    const loadReviewData = async () => {
      ...
      try {
        ...
        // Load review
        const reviewData = await getAttractionReviewById(parseInt(reviewId));
        ...
        // Load replies
        const repliesData = await getReviewReplies("attraction", parseInt(reviewId), currentUserId);
        setReplies(repliesData);

        // Load review like count and status
        const likeCount = await getLikeCount("attraction", parseInt(reviewId));
        setReviewLikeCount(likeCount);

        const isLiked = await checkIsLiked(currentUserId, "attraction", parseInt(reviewId));
        ...
        {review.aspects && (
          <div className="bg-gray-50 rounded-lg p-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                { label: "Trải nghiệm", value: review.aspects.experience },
                { label: "Giá trị so với chi phí", value: review.aspects.valueForMoney },
                { label: "Khả năng tiếp cận", value: review.aspects.accessibility },
                { label: "Tiện nghi", value: review.aspects.facilities },
                { label: "Nhân viên", value: review.aspects.staff },
              ].map((aspect) => (
