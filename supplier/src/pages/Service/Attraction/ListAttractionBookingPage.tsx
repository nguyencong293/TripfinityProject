...
import { getAttractionsByProvider } from "../../../services/attractionService";
import ConfirmModal from "../../../components/common/ConfirmModal";

const ListAttractionBookingPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load attractions and bookings
        const [attractionsData, bookingsRes] = await Promise.all([
          getAttractionsByProvider(provider.providerId),
          api.get<AttractionBookingDTO[]>(`/attraction-bookings/provider/${provider.providerId}`)
        ]);
  ...
  const columns: Array<{ key: ColumnKey; label: string }> = useMemo(
    () => [
      ...
      { key: "attractionName", label: "Điểm tham quan" },
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      if (userStr) {
        ...
        const provider = await getProviderByUserId(user.userId);
        if (provider?.providerId) {
          const bookingsRes = await api.get<AttractionBookingDTO[]>(`/attraction-bookings/provider/${provider.providerId}`);
  ...
  const cell = (b: AttractionBookingDTO, key: ColumnKey): React.ReactNode => {
    ...
    switch (key) {
      ...
      case "attractionName":
        return attraction?.title || `Attraction ID: ${b.attractionId}`;
      case "userName":
        return user?.fullName || `User ID: ${b.userId}`;
      case "userPhone":
        return user?.phoneNumber || "-";
      case "visitDate":
        return formatDate(b.startDate || b.visitDate);
      case "numAdults":
        return `${b.numAdults} người`;
      case "totalPrice":
        return (
      ...
      case "bookingStatus": {
        ...
        const providerLabel = b.providerConfirmed === 0 ? "Chờ xác nhận" : b.providerConfirmed === 1 ? "Đã xác nhận" : "Đã hủy";
        return (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-sm ${dark ? "text-gray-400" : "text-gray-600"}`}>
                {bookings.filter(b => b.providerConfirmed === 0).length} đơn chờ xác nhận
              </p>
            </div>
          </div>
        </div>
                ...
                <tr>
                  <td colSpan={columns.length} className="px-4 py-8 text-center">
                    ...
                    <p className={dark ? "text-gray-400" : "text-gray-500"}>
                      {activeTab === 0 && "Chưa có đơn chờ xác nhận"}
                      {activeTab === 1 && "Chưa có đơn đã xác nhận"}
                      {activeTab === 2 && "Chưa có đơn đã hủy"}
                    </p>
                  </td>
                </tr>
              ) : (
...
export default ListAttractionBookingPage;
