...
import type { AttractionBookingDTO, AttractionDTO, UserDTO } from "../../../types";
import api from "../../../services/api";
import { getUserById } from "../../../services/providerService";
import ConfirmModal from "../../../components/common/ConfirmModal";

const AttractionBookingViewPage: React.FC = () => {
  const { bookingId } = useParams<{ bookingId: string }>();
  const navigate = useNavigate();
  const { t } = useLanguage();
  
  const [booking, setBooking] = useState<AttractionBookingDTO | null>(null);
  ...
  useEffect(() => {
    ...
    const loadBookingDetails = async () => {
      try {
        ...
        const bookingRes = await api.get<AttractionBookingDTO>(`/attraction-bookings/${bookingId}`);
        const bookingData = bookingRes.data;
        setBooking(bookingData);

        // Fetch attraction details
        if (bookingData.attractionId) {
          const attractionRes = await api.get<AttractionDTO>(`/attractions/${bookingData.attractionId}`);
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      if (modalState.type === "confirm") {
        await api.patch(`/attraction-bookings/${bookingId}/confirm`);
      } else {
        await api.patch(`/attraction-bookings/${bookingId}/cancel`);
      }
    ...
    return (
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="rounded-xl border theme-border theme-bg-card p-6 text-center">
          ...
          <p className="text-lg theme-text-primary mb-4">{error || "Không tìm thấy thông tin đặt vé"}</p>
          <button
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold theme-text-primary">
              Chi tiết đặt vé #{booking.bookingId}
            </h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-xl border theme-border theme-bg-card p-6">
            <div className="flex items-center gap-2 mb-4">
              <MapPin className="w-5 h-5 icon-brand" />
            ...
            {attraction ? (
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold theme-text-primary text-lg">{attraction.title}</h3>
              ...
              <p className="theme-text-secondary">Đang tải thông tin điểm tham quan...</p>
            )}
          </div>
          ...
          <div className="rounded-xl border theme-border theme-bg-card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm theme-text-secondary">Ngày tham quan</p>
                  <p className="font-medium theme-text-primary">{formatDate(booking.startDate || booking.visitDate)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Users className="w-5 h-5 theme-text-secondary" />
                <div>
                  <p className="text-sm theme-text-secondary">Số người</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm theme-text-secondary">Ngày đặt</p>
                  <p className="font-medium theme-text-primary">{formatDateTime(booking.bookingDate)}</p>
                </div>
              </div>
              {booking.providerNotes && (
                <div className="col-span-2 flex items-start gap-3">
                  <AlertCircle className="w-5 h-5 theme-text-secondary mt-0.5" />
                  <div>
                    <p className="text-sm theme-text-secondary">Ghi chú</p>
                    <p className="font-medium theme-text-primary">{booking.providerNotes}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {booking.providerConfirmed === 1 && booking.providerConfirmedAt && (
            <div className="rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 p-6">
              ...
              <p className="text-sm text-green-700 dark:text-green-300">
                Xác nhận lúc: {formatDateTime(booking.providerConfirmedAt)}
              </p>
            </div>
          )}
...
export default AttractionBookingViewPage;
